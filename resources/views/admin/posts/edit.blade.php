            ...
            <div class="card-body">
                <form method="post" action="{{route('posts.update',['post'=>$post->id])}}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" class="form-control">
                            @foreach($categories as $key=>$cat)
                                <option @if($key==$post->category_id) selected @endif  value="{{$key}}">{{$cat}}</option>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <select name='tags[]' class="select2" multiple="multiple" data-placeholder="теги" style="width: 100%;">
                            @foreach($tags as $key=>$cat)
                                <option  @if(in_array($key,$post->tags->pluck('id')->all())) selected @endif    value="{{$key}}">{{$cat}}</option>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <div><img width='90' hight="90" src="{{$post->getImage()}}" ></div>
                    </div>
